      ...
      cfg.group.forEach((groupTitle, i) => {
        ...
        if (issues && issues.length > 0) {
          if (groupTitle.length > 0) {
            $(el).append('<h2 class="link-title"><span class="fake-title">' + groupTitle + '</span></h2>');
          } else if (name == '' && groupTitles.length > 1) {
            $(el).append('<h2 class="link-title"><span class="fake-title">' + '未分组' + '</span></h2>');
          }
          $(el).append('<div class="link-container ' + i + '"></div>');
          // layout items
          for (j = 0; j < issues.length; j++) {
            let issue = issues[j];
            let imgTag = '';
            if (issue.img && issue.img.length > 0) {
              imgTag = '<div class="img"><img src="' + issue.img + '" onerror="javascript:this.src=\'https://cdn.jsdelivr.net/gh/volantis-x/cdn-wallpaper-minimalist/2020/052.jpg\';"/></div>';
            } else {
            ...
            infoTag += '<span class="title">' + issue.title + '</span><div class="desc">' + issue.description + '</div></div>';
            let cardTag = '<li class="link-item">';
            cardTag += "<a class='link-item-inner' target='_blank' href='" + issue.url + "'>" + imgTag + infoTag + "</a></li>";
            $(el).find('.link-container.' + i).append(cardTag);
          }
        }
      });
    }, function() {
